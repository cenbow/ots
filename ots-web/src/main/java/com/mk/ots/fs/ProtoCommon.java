...
public class ProtoCommon {
	...
	public static String getToken(String remote_filename, int ts,
			...
			NoSuchAlgorithmException, FSException {
	...
	public static String genSlaveFilename(String master_filename,
			String prefix_name, String ext_name) throws FSException {
		String true_ext_name;
		int dotIndex;

		if (master_filename.length() < 28 + FDFS_FILE_EXT_NAME_MAX_LEN) {
			throw new FSException("master filename \"" + master_filename
					+ "\" is invalid");
		}
		...
		if (true_ext_name.length() == 0 && prefix_name.equals("-m")) {
			throw new FSException("prefix_name \"" + prefix_name
					+ "\" is invalid");
		}
