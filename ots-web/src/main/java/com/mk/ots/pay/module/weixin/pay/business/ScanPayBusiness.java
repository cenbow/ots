package com.mk.ots.pay.module.weixin.pay.business;

import java.awt.Window.Type;
import com.mk.ots.pay.module.weixin.pay.common.Configure;
import com.mk.ots.pay.module.weixin.pay.common.Log;
import com.mk.ots.pay.module.weixin.pay.common.Signature;
import com.mk.ots.pay.module.weixin.pay.common.Util;
import com.mk.ots.pay.module.weixin.pay.common.WxType;
import com.mk.ots.pay.module.weixin.pay.common.report.ReporterFactory;
...
import com.mk.ots.pay.module.weixin.pay.protocol.pay_protocol.ScanPayResData;
...
import com.mk.ots.pay.module.weixin.pay.protocol.reverse_protocol.ReverseResData;
import com.mk.ots.pay.module.weixin.pay.service.ReverseService;
import com.mk.ots.pay.module.weixin.pay.service.ScanPayQueryService;
import com.mk.ots.pay.module.weixin.pay.service.ScanPayService;
...
public class ScanPayBusiness {
    ...
    public ScanPayBusiness(WxType type) throws IllegalAccessException, ClassNotFoundException, InstantiationException {
        scanPayService = new ScanPayService(type);
        scanPayQueryService = new ScanPayQueryService(type);
        reverseService = new ReverseService(type);
    }
     ...
     * @throws Exception
     */
    public void run(ScanPayReqData scanPayReqData, ResultListener resultListener,WxType type) throws Exception {
        ...
        log.i("支付API返回的数据如下：");
        payServiceResponseString = scanPayService.request(scanPayReqData,type);
        ...
        if (Configure.isUseThreadToDoReport()) {
            ReporterFactory.getReporter(reportReqData).run(type);
            timeAfterReport = System.currentTimeMillis();
            log.i("pay+report总耗时（异步方式上报）：" + (timeAfterReport - costTimeStart) + "ms");
        } else {
            ReportService.request(reportReqData,type);
            timeAfterReport = System.currentTimeMillis();
        ...
        if (scanPayResData.getReturn_code().equals("FAIL")) {
            ...
            //--------------------------------------------------------------------
            if (!Signature.checkIsSignValidFromResponseString(payServiceResponseString,type)) {
            ...
            if (scanPayResData.getResult_code().equals("SUCCESS")) {
                ...
                if (errorCode.equals("AUTHCODEEXPIRE") || errorCode.equals("AUTH_CODE_INVALID") || errorCode.equals("NOTENOUGH")) {
                    ...
                    if (doPayQueryLoop(payQueryLoopInvokedCount, outTradeNo,type)) {
     ...
     * @throws Exception
     */
    private boolean doOnePayQuery(String outTradeNo,WxType type) throws Exception {

        sleep(waitingTimeBeforePayQueryServiceInvoked);//等待一定时间再进行查询，避免状态还没来得及被更新

        String payQueryServiceResponseString;

        ScanPayQueryReqData scanPayQueryReqData = new ScanPayQueryReqData("",outTradeNo,type);
        payQueryServiceResponseString = scanPayQueryService.request(scanPayQueryReqData,type);
     ...
     */
    private boolean doPayQueryLoop(int loopCount, String outTradeNo,WxType type) throws Exception {
        //至少查询一次
        if (loopCount == 0) {
            loopCount = 1;
        }
        ...
        for (int i = 0; i < loopCount; i++) {
            if (doOnePayQuery(outTradeNo,type)) {
                return true;
            }
        }
        return false;
     ...
     * @throws Exception
     */
    private boolean doOneReverse(String outTradeNo,WxType type) throws Exception {

        sleep(waitingTimeBeforeReverseServiceInvoked);//等待一定时间再进行查询，避免状态还没来得及被更新

        String reverseResponseString;

        ReverseReqData reverseReqData = new ReverseReqData("",outTradeNo,type);
        reverseResponseString = reverseService.request(reverseReqData,type);
     ...
     * @throws InterruptedException
     */
    private void doReverseLoop(String outTradeNo,WxType type) throws Exception {
        //初始化这个标记
        needRecallReverse = true;
        //进行循环撤销，直到撤销成功，或是API返回recall字段为"Y"
        while (needRecallReverse) {
            if (doOneReverse(outTradeNo,type)) {
                return;
            }
        }
    }
