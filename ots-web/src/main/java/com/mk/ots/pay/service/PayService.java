...
public class PayService implements IPayService {
    ...
    @Autowired
    private IPScoreLogDao ipScoreLogDao;
    @Autowired
    ...
    private static final String REMIND_YF = "恭喜您预订成功！请在预抵时间前抵达并办理入住， 如需延迟到店请联系酒店前台！如遇问题联系眯客客服：80823502";
    private static final String REMIND_PT = "恭喜您预订成功！请在预抵时间前抵达并办理入住。记得下次直接在线预定哦，比到店办理更便宜！";
    private static final String REMIND_COMMON = "【眯客imike】";
    private static final String REMIND_URL = "www.imikeshareMessage-url-scheme://orderlist";

    //EHotel中标示是否PMS2.0的字段
  	private static final String IS_NEWPMS4EHOTEL = "isNewPms";
...
//            POrderLog orderLog = ipOrderLogDao.findPOrderLogByPay(pay.getId());
            if(type == PPayInfoTypeEnum.REFUNDLEZHU4LONG || type == PPayInfoTypeEnum.REFUNDLEZHU4QIEKE) {
                logger.info("订单:" + orderId + type.getName());
//                addPPayinfo(pay, orderLog.getRealotagive().negate(), null, type,null);
//                orderLog.setOtagive(new BigDecimal(0));
//                orderLog.setRealotagive(new BigDecimal(0));
//                
...
//                logger.info("订单:" + orderId + "平账完成.");
    ...
    public boolean pmsAddpayOk(OtaOrder order, PayStatusEnum payStatus, Boolean ispay,Long pmsSendId) {
        ...
        if ((ispay != null) && ispay) {
            ...
            if (payStatus.getId().intValue() == PayStatusEnum.alreadyPay.getId().intValue()) {
                price = orderLog.getUsercost().add(orderLog.getRealotagive());
    ...
    public boolean cancelPmsPay(Long orderId, PPayInfoTypeEnum type) {
        ...
        if (findPOrderLogByPay.getPmssend() == PmsSendEnum.ResponseSuccess && findPOrderLogByPay.getPmsrefund() != PmsSendEnum.ResponseSuccess) {
            ...
            if(cancelPayToPms(order, pay, refundPrice, findPOrderLogByPay.getId(),pmsSendIdLong)) {
                ...
                findPOrderLogByPay.setOtagive(BigDecimal.ZERO);
	...
	private void cancelPayToPmsV2(OtaOrder order, PPay pay, POrderLog orderLogByPay, PPayInfoTypeEnum type) {
		...
		try {
			...
			String responseString = Tools.dopostjson(PMS_PAY_CANCEL_URL, request);
	...
	private String wrapPMSRequest(OtaOrder order, long payid, BigDecimal price, String function){
		...
		res.setUuid(Tools.getUuid());
		res.setFunction(function);
		res.setTimestamp(Tools.getTimestamp());
    ...
    private BigDecimal calculateRefundLeZhu(OtaOrder order, POrderLog orderLogByPay, PPayInfoTypeEnum type) {
        ...
        if (OrderTypeEnum.YF.getId().intValue() == order.getOrderType()) {//预付
            ...
            if(type == PPayInfoTypeEnum.Y2U) {//正常取消乐住币:用户支付+优惠补贴
                ...
                price = orderLogByPay.getAllcost();
	...
	private  boolean  pmsAddpayV2(OtaOrder order, long payid, long pmsSendId, BigDecimal price, String operateName) {
		...
		try {
			...
			String responseString = Tools.dopostjson(PMS_PAY_ADD_URL, request);
...
//           this.sendLeZhuFail(order, payid, pmsSendId,price, memberName);
	...
	private void sendLeZhuFail(OtaOrder order, long payid,Long pmsSendId, BigDecimal price, String memberName){
		Long orderId = order.getId();
		this.logger.info("订单:" + orderId + " 准备添加redis ");
		if (this.manager == null) {
			this.manager = AppUtils.getBean(OtsCacheManager.class);
		}
		ReSendLeZhu reSendLeZhu = null;
		Jedis jedis = this.manager.getNewJedis();
		 logger.info("reSendLeZhuList的长度:"+jedis.llen("reSendLeZhuList"));
		try {
			reSendLeZhu = new ReSendLeZhu(order, payid, pmsSendId,price, memberName);
			logger.info(JsonKit.toJson(reSendLeZhu));
			jedis.lpush("reSendLeZhuList",JsonKit.toJson(reSendLeZhu));
		} catch (Exception e) {
			logger.error("插入redis异常",e);
		} finally {
			jedis.close();
		}
		 logger.info("reSendLeZhuList的长度:"+jedis.llen("reSendLeZhuList"));
		 this.logger.info("订单:" + orderId + " 准备添加redis完成 ");
	}
	...
	private void sendLeZhuFailWithDB(OtaOrder order, long payid,Long pmsSendId, BigDecimal price, String memberName){
		...
		try {
			reSendLeZhu = new ReSendLeZhu(order, payid, pmsSendId,price, memberName);
			String json2ReSendLeZhu = JSON.toJSONString(reSendLeZhu);
...
//          parse.getTicket().getPromotion().getType()
     ...
     */
    private BigDecimal saveOrderLog(PPay pay, BigDecimal allcost, BigDecimal realCost, List<ITicketParse> ticketParses, List<ITicketParse> promotionParses) {
        POrderLog orderLog = this.ipOrderLogDao.findPOrderLogByPay(pay.getId());
        if (orderLog == null) {
        	return this.createOrderLog(pay, allcost, realCost, ticketParses, promotionParses);
        }
        this.updateOrderLog(ticketParses, promotionParses, orderLog, pay, realCost);
     ...
     * @param ticketParse
     */
    private BigDecimal createOrderLog(PPay pay, BigDecimal allcost, BigDecimal realCost, List<ITicketParse> ticketParses, List<ITicketParse> promoParses) {
        POrderLog orderLog = new POrderLog();
        this.calOrderLogSubsidy(ticketParses, promoParses, orderLog, BigDecimal.ZERO, BigDecimal.ZERO, allcost, BigDecimal.ZERO);
     ...
     * @param promotionParses
     */
    private void updateOrderLog(List<ITicketParse> ticketParses, List<ITicketParse> promoParses, POrderLog orderLog, PPay pay, BigDecimal realCost) {
        ...
        this.calOrderLogSubsidy(ticketParses, promoParses, orderLog, hotelSubsidy, otaSubsidy, userCost, realOtagive);
        orderLog.setPayid(pay.getId());
    ...
    private void calOrderLogSubsidy(List<ITicketParse> ticketParses, List<ITicketParse> promoParses, POrderLog orderLog, BigDecimal hotelSubsidy, BigDecimal otaSubsidy,
            BigDecimal allCost, BigDecimal realOtagive) {
        ...
        if(CollectionUtils.isNotEmpty(list)){
            setOrderLog(list, orderLog, hotelSubsidy, otaSubsidy, allCost, realOtagive);
        }else{
            orderLog.set("usercost", allCost);
            orderLog.set("hotelgive", hotelSubsidy);
            orderLog.set("otagive", otaSubsidy);
            orderLog.set("realotagive", realOtagive);

            orderLog.setUsercost(allCost);
            orderLog.setOtagive(otaSubsidy);
            orderLog.setHotelgive(hotelSubsidy);
            orderLog.setRealotagive(realOtagive);
        }
    }
    ...
    private  void setOrderLog(List<ITicketParse> list, POrderLog orderLog, BigDecimal hotelSubsidy, BigDecimal otaSubsidy,
        ...
        orderLog.set("usercost", userCost);
        orderLog.set("hotelgive", hotelSubsidy);
        orderLog.set("otagive", otaSubsidy);
        orderLog.set("realotagive", realOtagive);
	...
	public void pushMsg(Long orderId, String paytype) {
		...
		if (StringUtils.isNotBlank(paytype)) {
			...
			if (paytype.trim().equals("1")) {
				...
				this.iMessageService.pushMsg(order.getContactsPhone(), PayService.REMIND_COMMON, PayService.REMIND_YF, MessageType.USER.getId(), REMIND_URL);
			} else if (paytype.trim().equals("2")) {

				this.logger.info("订单:" + orderId + "到付订单推送消息.");
				this.iMessageService.pushMsg(order.getContactsPhone(), PayService.REMIND_COMMON, PayService.REMIND_PT, MessageType.USER.getId(), REMIND_URL);
			}
		} else {

			this.logger.info("订单:" + orderId + "退款推送消息.");
			String msg = "您已取消" + this.getTime(order.getBeginTime()) + "“（" + order.getHotelName() + "）”的订单，我们将在3天内将预付款退到您的原账户中，请耐心等待~ 如遇问题联系眯客客服：80823502";

			this.iMessageService.pushMsg(order.getContactsPhone(), PayService.REMIND_COMMON, msg, MessageType.USER.getId(), REMIND_URL);
		}
	}
...
//		}
		...
		BigDecimal realOtaGive = this.saveOrderLog(pay, allcost, realCost, ticketParses, promotionParses);
		// 更新info的realcost
		this.logger.info("订单:" + orderId + "realOtaGive:" + realOtaGive + ",ticketPayInfos的长度" + ticketPayInfos.size() + ",promotionPayInfos的长度" + promotionPayInfos);
		...
		if ((countPayInfo != null) && (countPayInfo.size() > 0)) {
			this.logger.info("订单:" + orderId + "PPayinfo中的realCost更新为Realotagive开始");
			this.updatePPayInfoRealCost(countPayInfo, realOtaGive);
			this.logger.info("订单:" + orderId + "PPayinfo中的realCost更新为Realotagive完毕");
		}
	 ...
	 * @param realotagive
	 */
	private void updatePPayInfoRealCost(List<PPayInfo> ticketPayInfos, BigDecimal realotagive) {
		PPayInfo payInfo = ticketPayInfos.get(0);
		payInfo.setRealCost(realotagive);
	...
	private Map<String, Object> weixinpay(HttpServletRequest request, long orderid, BigDecimal amount, String body, String notifyUrl) {
		...
		if (!iswechat) {
			...
			try {
				// 把价格设置成【分】
				int price = Tools.get100price(amount);
...
//		this.iPPmsPayLogDao.logpay(orderid, lezhu, reason, operatorid);
	...
	public Boolean sendMsg(OtaOrder order){
        ...
        if(roomOrder!=null){
            ...
            if(hg!=null && user!=null){
                ...
                //2. 发送短信
                Long msgid = iMessageService.logsms(phone, msgContent, MessageTypeEnum.normal, "", "");
    ...
    private void sendMsg2Sale(OtaOrder order) {
        ...
        try {
            ...
            String response = Tools.dopost(PayService.INFORM_SALE_URL + orderId, "");
    ...
    private boolean bankRefund(Long orderid, PPay pay,List<PPayInfo> payinfolist ) {
        ...
        if (bankType == PPayInfoOtherTypeEnum.alipay) {
            ...
            int m = Tools.get100price(refundPrice);
            if (m > 0) {
    ...
    private boolean isNot0AndNull(BigDecimal price){
  	...
  	public String  findPay(String orderid){
  		...
  		String rs=QueryPay.findPay(orderid);
...
//			String request="{\"hotelid\":\""+hotel.get("pms")+"\",\"customerid\":\""+pmsRoomOrderNo+"\"}";
			...
			try {
				responseString = Tools.dopostjson(PMS_FIND_PAYINFO, request);
			} catch (Exception e) {
				e.printStackTrace();
			} 
		}
	...
	public String cancelpaybyerror(Long orderid) {
		...
		if (hotel != null) {
			...
			if (cost != BigDecimal.ZERO) {
				...
				try {
					responseString = Tools.dopostjson(PMS_CANCEL_PMS_URL, request);
	...
	public boolean addQiekeIncome(OtaOrder order, RuleEnum rule) {
		...
		try {
			...
			BigDecimal bd = this.getRuleValue(order.getOrderType(), rule);
	...
	private boolean  cancelCouponPmsV2(OtaOrder order, Long payinfoid,   BigDecimal price) {
		...
		try {
			...
			String responseString = Tools.dopostjson(PMS_CANCEL_PMS_URL, request);
			response = JSON.parseObject(responseString, PMSResponse.class);
		} catch (Exception e) {
	...
	/** 从数据库查询补贴的 */
	private BigDecimal getRuleValue(int ordertype, RuleEnum rule) throws Exception {
		BAreaRuleDetail ar = null;
		if (ordertype == OrderTypeEnum.YF.getId().intValue()) {
			ar = this.areaRuleDetailService.queryRuleValue(OrderTypeEnum.YF.name(), rule);
		} else if (ordertype == OrderTypeEnum.PT.getId().intValue()) {
			ar = this.areaRuleDetailService.queryRuleValue(OrderTypeEnum.PT.name(), rule);
		}
		if (ar != null) {
			return new BigDecimal(ar.getRulevalue());
		}
...
//			}
	...
	public String readAutoSend(String payid) {
		...
		try {
			   ...
			   logger.info("readAutoSend:reSendLeZhuList的长度:"+relen);
		       for (int i = 0; i < relen; i++) {
		        	  String str =jedis.rpop("reSendLeZhuList");
			          reSendLeZhu = JSON.parseObject(str,ReSendLeZhu.class);
			          if(payid.trim().length()!=0&&payid!=null){
					          if(payid.equals(String.valueOf(reSendLeZhu.getPayid()))){
					        	  logger.info("readAutoSend,"+reSendLeZhu.getPayid()+"###########");
					        	  logger.info("readAutoSend,"+JsonKit.toJson(reSendLeZhu));
					        	  reSBuffer.append(JsonKit.toJson(reSendLeZhu));
					          }else {
								logger.info("readAutoSend,"+reSendLeZhu.getPayid()+"***********");
					          }
				          } else {
			          ...
			          jedis.lpush("reSendLeZhuList", str);
	        }
