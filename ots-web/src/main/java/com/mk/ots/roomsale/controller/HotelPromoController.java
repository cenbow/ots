...
public class HotelPromoController {
	...
	@RequestMapping(value = "/promo/queryroomtypes", method = RequestMethod.POST)
	@ResponseBody
	public ResponseEntity<Map<String, Object>> queryRoomtypes(String hotelid, String roomid, String roomtypeid,
			String valid) {
		List<RoomPromoDto> result = null;

		try {
			TRoomSaleConfig config = new TRoomSaleConfig();
			config.setHotelId(Integer.parseInt(hotelid));
			config.setRoomId(Integer.parseInt(roomid));
			config.setRoomTypeId(Integer.parseInt(roomtypeid));
			config.setValid(valid);

			result = roomSaleService.queryRoomPromoByHotel(config);

			BeanMap bean = new BeanMap(result.get(0));

			logger.info("salename:%s; promotype:%s", result.get(0).getSaleName(), result.get(0).getPromoType());

			return new ResponseEntity<Map<String, Object>>(bean, HttpStatus.OK);
		} catch (Exception ex) {
			logger.error("failed to queryRoomPromoInfoByHotel", ex);
		...
		try {
			...
			if (CollectionUtils.isNotEmpty(roomSaleConfigInfoList)) {
				for (TRoomSaleConfigInfo saleConfigInfo : roomSaleConfigInfoList) {
					long sec = calDiffTime(saleConfigInfo.getStartDate(), saleConfigInfo.getEndDate(),
