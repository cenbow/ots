...
//      }
    ...
    private void cancelPmsOrder(OtaOrder order) {
    ...
    private void  returnTicketOrderPay(OtaOrder order){
        //用户入住之前允许退还优惠券给用户  上海规则
        if(order.getRuleCode()==null||RuleEnum.SHANG_HAI.getId().equals(order.getRuleCode())){
            logger.info("规则："+order.getRuleCode());
...
//                  checkInUser.put("ADDRESS", otaCheckInUser.get("Address", ""));
        ...
        if (otaCheckInUserList != null && otaCheckInUserList.size() > 0) {
            ...
            for (OtaCheckInUser otaCheckInUser : otaCheckInUserList) {
                ...
                if (StringUtils.isNotBlank(otaCheckInUser.getStr("Phone"))) {
                    pmsCheckInPerson.setPhoto(otaCheckInUser.getStr("Phone"));
                }
            }
	...
	public OtaRoomOrder saveRoomOrder(OtaOrder order, OtaRoomOrder newRoomOrder, boolean isQieKe) {
		...
		try {
			...
			// 多查询一天saveOtaRoomPriceByOtaRoomOrder方法使用
			// Calendar cal = Calendar.getInstance();
			// cal.setTime(DateUtils.getDateFromString(newRoomOrder.getStr("endtime")));
			// cal.add(Calendar.DATE, 1);
			Date begin = newRoomOrder.getDate("begintime");
			Calendar calNow = Calendar.getInstance();
			calNow.setTime(begin);
			if (calNow.get(calNow.HOUR_OF_DAY) < 4) {
				begin = DateUtils.addHours(begin, -6);
			}
			if (calNow.get(calNow.HOUR_OF_DAY) >= 4 && calNow.get(calNow.HOUR_OF_DAY) < 18) {
				begin = DateUtils.getDateFromString(DateUtils.getStringFromDate(begin, DateUtils.FORMATSHORTDATETIME) + "180000");
			}
			Date end = newRoomOrder.getDate("endtime");// DateUtils.addDays(newRoomOrder.getDate("endtime"),
														// 1);
			Bean roomType = hotelDAO.findRoomTypeByRoomtypeid("" + newRoomOrder.getLong("roomtypeid"));
			List<Date> dateList = DateTools.getBeginDateList(begin, end);
			Map<String, BigDecimal> map = roomService.getCost(newRoomOrder.getLong("otaorderid"), roomType, newRoomOrder.getLong("hotelid"),
					newRoomOrder.getLong("roomtypeid"), newRoomOrder.getLong("mid"), dateList, isQieKe);
    ...
    public OtaOrder calcOrderCost(OtaOrder order) {
        ...
        for (OtaRoomOrder otaRoomOrder : order.getRoomOrderList()) {
            ...
            otaRoomOrder.set("totalprice", this.getTotalCostByOtaRoomOrder(otaRoomOrder, order.getSpreadUser() != null));
    ...
    public BigDecimal getTotalCostByOtaRoomOrder(OtaRoomOrder roomOrder, boolean isQieKe) {
        ...
        Date begin = roomOrder.getDate("begintime");
        Calendar calNow = Calendar.getInstance();
        calNow.setTime(begin);
        if (calNow.get(calNow.HOUR_OF_DAY) < 4) {
            begin = DateUtils.addHours(begin, -6);
        ...
        Date end = roomOrder.getDate("endtime");// DateUtils.addDays(roomOrder.getDate("endtime"),
                                                // 1);
        Bean roomType = hotelDAO.findRoomTypeByRoomtypeid("" + roomOrder.getLong("roomtypeid"));
        List<Date> dateList = DateTools.getBeginDateList(begin, end);
        Map<String, BigDecimal> map = roomService.getCost(roomOrder.getLong("otaorderid"), roomType, roomOrder.getLong("hotelid"),
                roomOrder.getLong("roomtypeid"), roomOrder.getLong("mid"), dateList, isQieKe);
        if ((map != null) && (map.size() > 0)) {
            for (Map.Entry<String, BigDecimal> entry : map.entrySet()) {
                BigDecimal price = entry.getValue();
                if (price != null) {
...
//								this.payService.checkinCancelCoupon(otaorder);
                ...
                if(otaorder.getOrderStatus()!=OtaOrderStatusEnum.CheckIn.getId()){
                    ...
                    pushMessage.setPhone(otaorder.getContactsPhone());
                    pushMessage.setTitle("入住10分钟提醒");
                    pushMessage.setMsgContent("您已入住成功！赶紧去眯一会吧~ 如遇问题联系眯客客服：80823502");
                    
                    //发送信息
                    orderTasts=new OtaOrderTasts();
                    orderTasts.setCount(0);
                    orderTasts.setCreatetime(new Date());
                    orderTasts.setUpdatetime(new Date());
                    orderTasts.setContent(JsonKit.toJson(pushMessage));
                ...
                if (pmsRoomOrder.get("CheckInTime") != null && pmsRoomOrder.get("CheckOutTime") != null
                        && OrderTypeEnum.PT.getId().intValue() == otaorder.getOrderType()) {
                ...
                PushMessage pushMessage = new PushMessage();
                pushMessage.setPhone(otaorder.getContactsPhone());
                pushMessage.setTitle("离店提醒");
                pushMessage.setMsgContent("感谢您使用眯客订房，如需续住时间，可继续使用眯客预定，在线支付更优惠哦！");
                ...
                orderTasts.setContent(JsonKit.toJson(pushMessage));
                // 马上发送
                orderTasts.setExecuteTime(new Date());
    ...
    private void checkOrdersByMid(OtaOrder order) {
        ...
        logger.info("OTSMessage::OrderController::createOrder::checkOrdersByMid:{},startTime:{},endTime:{}", mid, DateUtils.getDatetime(startTime),
                DateUtils.getDatetime(endTime));
        Long c = orderDAO.findOrderCountByMid(mid, startTime, endTime);
        logger.info("OTSMessage::OrderController::createOrder::checkOrdersByMid:{}", c);
        if (c != null && c.intValue() > 0) {
            throw MyErrorEnum.customError.getMyException("您今日已经预订过房间，不能再预订新的房间");
        }
    }
    ...
    private void checkInTimeBefore(OtaOrder order, THotel hotel, OtaRoomOrder tempRoom, boolean ifQieKe) {
        ...
        if(DateUtils.getDateFromString(beginTime).after(DateUtils.getDateFromString(endTime))){
            beginTime = tempRoom.get("begintime");
        }
    ...
    public void doModifyOrder(HttpServletRequest request, JSONObject jsonObj) {
        String orderId = request.getParameter("orderid");
        OtaOrder order = this.findOtaOrderById(Long.parseLong(orderId));
        if (order == null) {
            throw MyErrorEnum.findOrder.getMyException("订单号不存在");
        }
        OtaOrder pOrder = this.extractOrderBeanForModify(request, order);
        // 预付订单、等待支付的情况，计算房价
//		if (OrderTypeEnum.YF.getId().equals(order.getOrderType()) && order.getPayStatus() <= PayStatusEnum.waitPay.getId().intValue()) {
//		}
...
//                  checkInUser.put("idno", otaCheckInUser.get("Cardid", ""));
    ...
    private OtaOrder extractOrderBeanForModify(HttpServletRequest request, OtaOrder order) {
     ...
     * @param jsonObj
     */
    public void doCancelOrder(String orderid, JSONObject jsonObj) {
        logger.info("OTSMessage::取消订单cancelOrder:orderid:{}" + orderid);
...
//      String roomNo = request.getParameter("roomno");
        ...
        logger.info("开始创建订单::提取传递的参数::" + orderId + "::" + str.toString());
...
//            List<OtaRoomPrice> otaRoomPrices = priceService.findOtaRoomPriceByOrder(order);
    ...
    public void pushMessage(){
        ...
        for(OtaOrderTasts orderTasts:list){
            ...
            //推送消息
            Boolean result=iMessageService.pushMsg(
                    pushMessage.getPhone(),
                    pushMessage.getTitle(),
                    pushMessage.getMsgContent(),
//                  "订单号："+orderTasts.getOtaorderid()+pushMessage.getMsgContent(),
                    pushMessage.getMsgtype(),
                    "www.imikeshareMessage-url-scheme://orderlist");
            if(result){
                ...
                logger.info("用户关怀消息推送失败,orderid = {}",orderTasts.getOtaorderid());
            }
        }
   ...
   /* public void myTest(long otaorderid,String method){
    ...
    public void pushCheckInBefore1Msg(OtaOrder pOrder) {
        ...
        PushMessage pushMessage = new PushMessage();
        pushMessage.setTitle("到店时间提醒");
        pushMessage.setPhone(pOrder.getContactsPhone());
        pushMessage.setMsgContent("亲，离保留入住时间只有一小时啦，请尽快入住，过时订单将会自动取消哦~");
        String PmsUpdateOrderJson = JsonKit.toJson(pushMessage);

        OtaOrderTasts otaOrderTasts = new OtaOrderTasts();
        otaOrderTasts.setCount(0);
        otaOrderTasts.setCreatetime(new Date());
        otaOrderTasts.setExecuteTime(begintimeBefore1);
        otaOrderTasts.setContent(PmsUpdateOrderJson);
        otaOrderTasts.setHotelid(hotelId);
        otaOrderTasts.setOtaorderid(otaOrderid);
        otaOrderTasts.setStatus(OrderTasksStatusEnum.INITIALIZE.getId());
        otaOrderTasts.setTasktype(OrderTasksTypeEnum.ORDERPUSH.getId());
        otaOrderTasts.setUpdatetime(new Date());
    ...
    //发送 保留时间 取消订单的消息 
    public void pushOutCheckInTimeMsg(OtaOrder pOrder){//判断 只有前台到付的单子 需要发送消息
        if(pOrder.getOrderType() == OrderTypeEnum.PT.getId()){
            ...
            // 封装orderTasks 加到任务表中 status= 0初始化, taskType= 101push类型,
            PushMessage pushMessage = new PushMessage();
            pushMessage.setTitle("保留时间");
            pushMessage.setPhone(pOrder.getContactsPhone());
            pushMessage.setMsgContent("亲，由于未及时办理入住。订单可能会被取消，若仍需入住，请联系酒店协商。若已被取消，可再次使用眯客进行预订。");
            String PmsUpdateOrderJson = JsonKit.toJson(pushMessage);
            
            OtaOrderTasts otaOrderTasts = new OtaOrderTasts();
            otaOrderTasts.setCount(0);
            otaOrderTasts.setCreatetime(new Date());
            otaOrderTasts.setExecuteTime(pOrder.getBeginTime());
            otaOrderTasts.setContent(PmsUpdateOrderJson);
            otaOrderTasts.setHotelid(hotelId);
            otaOrderTasts.setOtaorderid(otaOrderid);
            otaOrderTasts.setStatus(OrderTasksStatusEnum.INITIALIZE.getId());
