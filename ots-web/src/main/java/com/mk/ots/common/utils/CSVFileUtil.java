...
public class CSVFileUtil {
     ...
     * @throws Exception
     */
    public static List readLine(String filename) throws Exception {
        ...
        while (bReadNext) {
            ...
            List row=fromCSVLinetoArray(strReadLine);
            list.add(row);
    ...
    public static void main(String[] args) throws Exception {
		...
		List lineList=util.readLine("d:\\demo.csv");
		List tableList = new ArrayList();
		for(int i=0;i<lineList.size();i++){
			List rowList= new ArrayList();
			rowList = util.fromCSVLinetoArray(lineList.get(i).toString());
			tableList.add(rowList);
		}
	
		List row1= (List)tableList.get(1);
		for(int i=0;i<row1.size();i++){
			System.out.println(row1.get(i).toString());
		}
		/*
