				...
				if (orderid != null) {
					...
					if (temp >= CancelOrderJob.CANCEL_ORDER_MAX_TIME_SPAN) {
						...
						if ((order != null) && (order.getOrderStatus() < OtaOrderStatusEnum.Confirm.getId())) {
							CancelOrderJob.logger.info("OTSMessage::CancelOrderJob: order:{}", order.getOrderStatus());
							try {
								...
								orderBusinessLogService.saveLog(order, OtaOrderFlagEnum.CANCELBYSYSTEM.getId(), "", "系统调度:订单15分钟后取消", "异常:"+e.getLocalizedMessage());
