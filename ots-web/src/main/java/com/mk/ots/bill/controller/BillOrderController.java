...
public class BillOrderController {
     ...
     * @return
     */
    @RequestMapping(value = "/genWeekClearing")
    public ResponseEntity<JSONObject> genWeekClearing(HttpServletRequest request) {
        String orderDate = request.getParameter("orderDate");
        String hotelid = request.getParameter("hotelid");
        if(orderDate==null){
            this.billOrderService.runtWeekClearing(null,null);
        }else{
            Date now=DateUtils.getDateFromString(orderDate);
            if(now==null){
                throw MyErrorEnum.customError.getMyException("日期错误，只需要某月的某日就可以。实例：2015-05-10,2015-06-10");
            }
            this.billOrderService.runtWeekClearing(now,hotelid);
        }
        JSONObject m = new JSONObject();
        m.put("sucess", true);
        return new ResponseEntity<JSONObject>(m, HttpStatus.OK);
     ...
     * @return
     */
    @RequestMapping(value = "/genchangeOrderStatusNoshow")
    public ResponseEntity<JSONObject> changeOrderStatusNoshow(HttpServletRequest request) {
        String orderDate = request.getParameter("orderDate");
        logger.info("更新订单状态为520，开始。orderDate: {}", orderDate);
        if(orderDate==null){
            this.billOrderService.changeOrderStatusNoshow(null);
        }else{
            Date now=DateUtils.getDateFromString(orderDate);
            if(now==null){
                throw MyErrorEnum.customError.getMyException("日期错误，只需要某月的某日就可以。实例：2015-05-10,2015-06-10");
            }
            this.billOrderService.changeOrderStatusNoshow(now);
        }
        JSONObject m = new JSONObject();
        m.put("sucess", true);
        return new ResponseEntity<JSONObject>(m, HttpStatus.OK);
