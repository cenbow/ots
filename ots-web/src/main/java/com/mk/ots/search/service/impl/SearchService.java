...
public class SearchService implements ISearchService {
    ...
    private Map<String, Object> readonlyOtsHotelFromEsStore(HotelQuerylistReqEntity reqentity) throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < hits.length; i++) {
                ...
                Long sales = Long.valueOf(String.valueOf(result.get("ordernummon") == null ? "0" : result.get("ordernummon")));
     ...
     */
    private void setMikepriceScriptSort(SearchRequestBuilder searchBuilder, BoolFilterBuilder boolFilter, List<String> mkPriceDateList) {
        // 脚本排序功能开始
     ...
     * @param sortType
     */
    private void setScoreScriptSort(SearchRequestBuilder searchBuilder, BoolFilterBuilder boolFilter,
            GeoPoint geopoint, List<String> mkPriceDateList) {
        // 脚本排序功能开始
        ScriptScoreFunctionBuilder scriptSortBuilder = new ScriptScoreFunctionBuilder();
    ...
    private void sortByRecommend(SearchRequestBuilder searchBuilder) {
        searchBuilder.addSort("ispms", SortOrder.ASC)
            .addSort("priority", SortOrder.DESC);
    }
    /**
     * 价格排序
     * @param searchBuilder
...
//    }
        ...
        try {
            ...
            if (StringUtils.isBlank(hotelid)) {
                ...
                if (StringUtils.isNotBlank(reqentity.getKeyword()) 
                        || StringUtils.isNotBlank(reqentity.getHotelname())
                        || StringUtils.isNotBlank(reqentity.getHoteladdr())) {
                    reqentity.setRange(SearchConst.SEARCH_RANGE_MAX);
                    logger.info("keyword or hotelname or hoteladdress search, set search range {}", SearchConst.SEARCH_RANGE_MAX);
                }
                ...
                if (HotelSearchEnum.BZONE.getId().equals(searchType) || 
                    ...
                    if (point != null) {
                        ...
                        logger.info("按照{}搜索，经纬度坐标：[{},{}], 搜索范围:{}米", 
                                HotelSearchEnum.getById(searchType).getName(), lon, lat, distance);
                    } else {
                        logger.error("按照{}搜索是没有获取到经纬度, points: {}。", HotelSearchEnum.getById(searchType).getName(), points);
                    }
                }
            ...
            for (int i = 0; i < hits.length; i++) {
                ...
                try {
                    ...
                    List<Map<String, String>> scores = thotelscoreMapper.findHotelScoresByHotelid(Long.valueOf(es_hotelid));
            	...
            	if(hotelPriceService.isUseNewPrice())
            		prices = hotelPriceService.getHotelMikePrices(Long.valueOf(es_hotelid), reqentity.getStartdateday(), reqentity.getEnddateday());
            	else
                	prices = roomstateService.getHotelMikePrices(Long.valueOf(es_hotelid), reqentity.getStartdateday(), reqentity.getEnddateday());
                Long endTime = new Date().getTime();
                Long times = endTime - startTime;
                ...
                Integer avlblroomnum = hotelService.getAvlblRoomNum(p_hotelid, p_isnewpms, p_visible, p_online,reqentity.getStartdateday(),reqentity.getEnddateday());
                endTime = new Date().getTime();
                times = endTime - startTime;
                ...
                if (Constant.STR_CITYID_SHANGHAI.equals(cityid)) {
                    ...
                    Long sales = result.get("ordernummon") == null ? 0l : Long.valueOf(String.valueOf(result.get("ordernummon")));
                    result.put("ordernummon", (sales >= 10 ? "月销" + sales + "单" : ""));
                ...
                if (hotelMap != null) {
                    if (Boolean.valueOf(String.valueOf(ServiceOutput.STR_MSG_SUCCESS))) {
                        result.put("service", hotelMap.get("service") == null ? new ArrayList<Map<String, Object>>() : hotelMap.get("service"));
                    } else {
        ...
        if (!StringUtils.isBlank(reqentity.getHotelname())) {
            QueryFilterBuilder hotelnameFilter = FilterBuilders
                    .queryFilter(QueryBuilders.matchQuery("hotelname",
                            reqentity.getHotelname()).operator(Operator.AND));
        ...
        if (reqentity.getHoteladdr() != null) {
            QueryFilterBuilder hoteladdrFilter = FilterBuilders
                    .queryFilter(QueryBuilders.matchQuery("detailaddr",
                            reqentity.getHoteladdr()).operator(Operator.AND));
    ...
    private void makeKeywordFilter(HotelQuerylistReqEntity reqentity, List<FilterBuilder> keywordBuilders) {
        try {
            ...
            if (!StringUtils.isBlank(reqentity.getKeyword())) {
                QueryFilterBuilder hotelNameFilter = FilterBuilders
                        .queryFilter(QueryBuilders.matchQuery("hotelname",
                                reqentity.getKeyword()).operator(Operator.AND));
    ...
    private List<FilterBuilder> makeMikePriceRangeFilter(HotelQuerylistReqEntity reqentity) throws ParseException {
        ...
        if (StringUtils.isNotBlank(reqentity.getMinprice()) || StringUtils.isNotBlank(reqentity.getMaxprice())) {
            ...
            while (startDateCal.compareTo(endDateCal) <= 0) {
                mikePriceBuilders.add(FilterBuilders.rangeFilter(SearchConst.MIKE_PRICE_PROP + sdf.format(startDateCal.getTime())).gte(Double.valueOf(minpriceParam)).lte(Double.valueOf(maxpriceParam)));
	...
	public Map<String, Object> readonlyPositionsFuzzy(String citycode, String keyword) {
		SearchRequestBuilder searchBuilder = esProxy.prepareSearch(esProxy.OTS_INDEX_DEFAULT, esProxy.POSITION_TYPE_DEFAULT);
		List<FilterBuilder> filterBuilders = new ArrayList<FilterBuilder>();
		QueryFilterBuilder citycodeFilter = FilterBuilders
				              .queryFilter(QueryBuilders.termQuery("citycode",citycode));
		filterBuilders.add(citycodeFilter);
		QueryFilterBuilder keywordFilter = FilterBuilders
								.queryFilter(QueryBuilders.multiMatchQuery(keyword, "name", "stations.stationname")
	...
	private List<SearchPositionsCoordinateRespEntity> readonlyHotelListFromES(String citycode, String keyword) {
		...
		QueryFilterBuilder keywordFilter = FilterBuilders.queryFilter(QueryBuilders.matchQuery("hotelname", keyword ).operator(Operator.AND));
		filterBuilders.add(keywordFilter);
		// 只搜索签约酒店
		filterBuilders.add(FilterBuilders.termFilter("ispms", 1));
    ...
    private Map<String, Object> addAreaDocs(String citycode) {
        ...
        try {
            ...
            if (esdatas.size() > 0) {
                esProxy.batchAddDocument(ElasticsearchProxy.OTS_INDEX_DEFAULT, ElasticsearchProxy.POSITION_TYPE_DEFAULT, esdatas);
                result.put(ServiceOutput.STR_MSG_SUCCESS, true);
            ...
            result.put(ServiceOutput.STR_MSG_ERRMSG, "citycode: " + citycode + ", addAreaDocs:: method error: " + e.getLocalizedMessage());
    ...
    private Map<String, Object> addLandmarkDocs(String citycode) {
        ...
        try {
            ...
            result.put(ServiceOutput.STR_MSG_SUCCESS, false);
            result.put(ServiceOutput.STR_MSG_ERRCODE, "-1");
            result.put(ServiceOutput.STR_MSG_ERRMSG, "citycode: " + citycode + ", addLandmarkDocs:: method error: " + e.getLocalizedMessage());
    ...
    private Map<String, Object> readonlySyncCityLandmarks(String citycode, boolean forceUpdate) {
        ...
        try {
            ...
            result.put(ServiceOutput.STR_MSG_SUCCESS, false);
            result.put(ServiceOutput.STR_MSG_ERRCODE, "-1");
            result.put(ServiceOutput.STR_MSG_ERRMSG, "citycode: " + citycode + ", readonlySyncCityLandmarks:: method error: " + e.getLocalizedMessage());
            logger.error("citycode: {}, readonlySyncCityLandmarks:: method error: {}", citycode, e.getLocalizedMessage());
        }
        return result;
    }
