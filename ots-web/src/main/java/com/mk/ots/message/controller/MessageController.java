...
public class MessageController {
	...
	public ResponseEntity<Map<String,Object>> sendVerifyCode(String phone, String type, String callmethod, String callversion, String ip,String citycoode) throws Exception {
		//1. 校验参数
        if(Strings.isNullOrEmpty(phone)){
            throw MyErrorEnum.errorParm.getMyException("[手机号码] 不允许为空.");
        }
        ...
        if (msgCount >= limitNum) {
			...
			String message=iMessageService.generateVerifyCode(phone);
		     ...
		     boolean sendMsg = iMessageService.sendCode(msgid, phone, message.trim(), messageTypeEnum,ip,citycoode);
		     logger.info("发送短信: phone:{}, message:{}, messagetype:{}, success:{}", phone, message.trim(), messageTypeEnum.getName(), sendMsg);
		     //3. 组织参数
			rtnMap.put("success", sendMsg);
		}
