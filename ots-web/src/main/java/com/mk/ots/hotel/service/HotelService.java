...
public class HotelService {
	...
	public ServiceOutput readonlyInitPmsHotel(String cityid, String thotelid) {
		// TODO: 有则忽略，无则添加
		/*select a.*,c.code as cityid, c.cityid as city_id,c.cityname,f.maxprice,f.minprice from t_hotel a
	    ...
	    left outer join (select hotelid,max(cost) as maxprice,min(cost) as minprice 
	                         from b_costtemp_310000 group by hotelid) f on a.id=f.hotelid
	    where a.visible = 'T' and a.online = 'T';
	    and a.id='555';*/
		...
		try {
            String sql = "select a.*,c.code as cityid, c.cityid as city_id,c.cityname,"
                    ...
                    + "     case when f.minprice is null then 0 else f.minprice end as minprice "
                    + " from t_hotel a "
                    + "     left outer join t_district b on a.disid = b.id "
                    + "       left outer join t_city c on b.cityid=c.cityid "
                    + "         left outer join (select hotelid,max(cost) as maxprice,min(cost) as minprice "
			...
			try {
				...
				for (THotelModel bean : hotels) {
					...
					//TODO: 待确认，先注释掉
					/*
					List<TRoomTypeInfoModel> rtfs = tRoomtypeInfoMapper.findByHotelid(Long.valueOf(hotelid));
					for(TRoomTypeInfoModel roomtypeinfo : rtfs){
						String rtPic = roomtypeinfo.getPics();
						if(StringUtils.isNotBlank(rtPic)){
							List<Map<String, Object>> rtp = (new Gson()).fromJson(rtPic, List.class);
							pics.addAll(rtp);
						}
					}
					*/
	...
	public ServiceOutput readonlyInitNotPmsHotel(String cityid, String thotelid) {
	    ...
	    try {
            ...
            for (Bean bean : hotels) {
                ...
                // 先把新的酒店放到集合中，后面做批量添加
                addList.add(hotel);
                logger.info("not pms hotelid: {} added in collections and will be add in elasticsearch document.", hotelid);
            }
            // 批量添加
            if (addList.size() > 0) {
	...
	public Bean readonlyHotelBaseInfo(String hotelID) {
		...
		StringBuffer bfSql = new StringBuffer();
		try {
			bfSql.append("select a.id,a.hotelphone,b.cityid,c.cityname as hotelcity,c.proid,d.proname as hotelprovince,e.disname as hoteldis"
					+ " from t_hotel a "
					+ " left outer join t_hotel_city b on a.id=b.hotelid"
					+ "   left outer join t_city c on b.cityid = c.code"
					+ "     left outer join t_province d on c.proid=d.proid "
					+ "       left outer join t_district e on e.id=a.disid"
					+ "         where a.id=?");
	...
	public Map<String, Object> readonlyOtsHotelFromEsStore(THotel hotel) throws Exception {
		...
		try {
            ...
            if (hotelDisid != null) {
                TDistrictModel tdistrictModel = tdistrictMapper.selectByPrimaryKey(Long.valueOf(hotelDisid.longValue()));
	...
	public Map<String, Object> readonlyOtsHotelListFromEsStore(THotel hotel) throws Exception {
		...
		try {
            ...
            if (StringUtils.isBlank(hotelid)) {
				...
				geoFilter.point(lat, lon).distance(distance, DistanceUnit.METERS).optimizeBbox("memory").geoDistance(GeoDistance.ARC);
                ...
                /** @version 2015-07-15 眯客2.1排序规则1: 是否在线, 酒店权重, 距离(距离我), 是否推荐(签约和非签约酒店), 最低价格, 酒店评分
                    @version 2015-07-24 排序规则2(废弃排序规则1) 排序规则排序规则：是否在线，权重，是否签约酒店，距离（由近及远）*/
                searchBuilder.addSort("online", SortOrder.DESC)
                    .addSort("priority", SortOrder.ASC)
                    .addSort("ispms", SortOrder.ASC)
                    .addSort(SortBuilders.geoDistanceSort("pin").point(lat, lon).order(SortOrder.ASC));  //根据屏幕经纬度 yub edit 20150724
            } else {
                // 如果输入参数中有hotelid，忽略其它过滤条件
                filterBuilders.add(FilterBuilders.termFilter("hotelid", hotelid));
            }
            ...
            for (int i = 0; i < hits.length; i++) {
                ...
                double hotelLongitude = Double.valueOf(String.valueOf(pin.get("lon")));
                double hotelLatitude = Double.valueOf(String.valueOf(pin.get("lat")));
                double hotelDistance = DistanceUtil.distance(lon, lat, hotelLongitude, hotelLatitude); //根据屏幕经纬度 yub 20150724
                result.put("distance", hotelDistance);
				...
				try {
					...
					if (!isHotelPic) {
						...
						/*
						 * 排序
						 * List<Map<String, String>> thebeans =(List<Map<String, String>>) result.get("hotelpic");
						
						Collections.sort(thebeans,new Comparator<Map<String, String>>(){  
				            @Override  
				            public int compare(Map<String, String> b1, Map<String, String> b2) {  
				                return b2.get("name").compareTo(b1.get("name"));  
				            }  
				        }); */
					...
					if (hotelInfo != null) {
						...
						result.put("hotelprovince", hotelInfo.get("hotelprovince") == null ? "" : hotelInfo.get("hotelprovince"));
						result.put("hotelphone", hotelInfo.get("hotelphone") == null ? "" : hotelInfo.get("hotelphone"));
					}
	...
	/**
	 * 最近预订时间查询
	 * @param createTime  yyyyMMddHHmmss
	 * @return
	 */
     ...
     * @param enddate
     */
    private void processRoomState(RoomstateQuerylistRespEntity.Room room, Long hotelid, String begindate, String enddate, Map<String, String> lockRoomsCache) throws Exception {
        ...
        for (int i = 0; i < lockDays; i++) {
            ...
            if (lockFlag != null) {
                ...
                this.logger.info("processRoomState::hotelid:{}, roomid:{}, Date: {}, status is nvc.", hotelid, roomid, lockDate);
    ...
    private void makeQueryFilter(THotel hotel, List<FilterBuilder> filterBuilders) {
        ...
        if (!StringUtils.isBlank(hotel.getHotelname())) {
            QueryFilterBuilder hotelnameFilter = FilterBuilders
                    .queryFilter(QueryBuilders.matchQuery("hotelname",
                            hotel.getHotelname()).operator(Operator.AND));
            filterBuilders.add(hotelnameFilter);
        }
        // hoteladdr模糊查询
        if (hotel.getHoteladdr() != null) {
            QueryFilterBuilder hoteladdrFilter = FilterBuilders
                    .queryFilter(QueryBuilders.matchQuery("detailaddr",
                            hotel.getHoteladdr()).operator(Operator.AND));
    ...
    private void makeKeywordFilter(THotel hotel, List<FilterBuilder> keywordBuilders) {
        try {
            ...
            if (!StringUtils.isBlank(hotel.getKeyword())) {
                QueryFilterBuilder hotelNameFilter = FilterBuilders
                        .queryFilter(QueryBuilders.matchQuery("hotelname",
                                hotel.getKeyword()).operator(Operator.AND));
    ...
    private List<FilterBuilder> makeMikePriceRangeFilter(THotel hotel) throws ParseException {
        ...
        if (StringUtils.isNotBlank(hotel.getMinprice()) || StringUtils.isNotBlank(hotel.getMaxprice())) {
            ...
            while (startDateCal.compareTo(endDateCal) <= 0) {
                mikePriceBuilders.add(FilterBuilders.rangeFilter(MIKE_PRICE_PROP + sdf.format(startDateCal.getTime())).gte(Double.valueOf(minpriceParam)).lte(Double.valueOf(maxpriceParam)));
	...
	public Bean readonlyHotelPrice(String cityid, String hotelid, String beginDate, String endDate) {
	    ...
	    try {
            ...
            if (bean == null || (bean.get("minprice") == null && bean.get("maxprice") == null)) {
                logger.info("!!!--== OTS Warning ==-- !!! cityid: {}, hotelid: {}, beginDate: {}, endDate: {} price not find.", cityid, hotelid, beginDate, endDate);
            } else {
                logger.info("!!!--== OTS Info ==-- !!! get Hotel: {} price success, maxprice = {}, minprice = {} .", hotelid, bean.get("maxprice"), bean.get("minprice"));
            }
        } catch (Exception e) {
	...
	private String readonlyRoomtypevc(Map<String, Object> roomtypeItem, THotel hotel) {
		...
		try {
            ...
            String sql = "select count(id) as counts from b_roomtemp_" 
                    + cityid + " where roomtypeid=?";
			String whereTime = "";
			if (!StringUtils.isBlank(whereStarttime)) {
				whereTime = whereStarttime;
			}
	...
	private List<Map<String, Object>> readonlyRoomtypeList(Map<String, Object> eshotel, String bedtype) {
		...
		try {
			StringBuffer bfSql = new StringBuffer();
            bfSql.append("select a.id as roomtypeid, a.name as roomtypename, a.cost as roomtypepmsprice, a.bednum,a.roomnum, "
	...
	private List<Map<String, Object>> readonlyRoomtypeFaciList(Map<String, Object> roomtypeItem) {
		...
		try {
	        String sql = "select a.facid as roomtypefacid,b.facname as roomtypefacname "
	                + " from t_roomtype_facility a "
	                + " left outer join t_facility b "
	                + " on a.facid = b.id "
	                + " where a.roomtypeid=? "
	                + " order by b.facsort asc";
	...
	private Object queryTransferData(Map<String, Object> data, THotel hotel) {
		...
		if (isRoomType) {
    		...
    		for (Map<String, Object> roomtypeItem : roomtypeList) {
    		    ...
    		    // roomtypevc
    		    // TODO: 获取房态信息: 该房间是否有可售.(由于现在房态信息缓存到redis，不再存储到mysql中间表，所以这里从缓存取.)
        		...
        		if (isRoomTypePic) {
        		    ////JSONArray pics = JSONArray.fromObject(roomtypeItem.get("pics"));
        		    ////List<Map<String, Object>> picsList = (List<Map<String, Object>>)JSONArray.toCollection(pics, Map.class);
        		    ObjectMapper objectMapper = new ObjectMapper();
        		    String pics = "";
        		    try {
        		...
        		BigDecimal roomtypeprice = roomstateService.getRoomPrice(Long.valueOf(hotelid), Long.valueOf(strRoomtypeid), beginDate, endDate);
	...
	public boolean syncNoPMSHotel(String name) throws Exception {
		...
		////String filename = this.getClass().getResource("/").getPath() + name;
    ...
    public List<Map<String, Object>> queryHistoryHotels(String token, String code,int start,int limit) throws Exception{
    	...
    	for (THotelModel tHotelModel:list) {
    		...
    		if(StringUtils.isNotEmpty(picJson)){
    			List<HotelPicJsonBean> beans =  gson.fromJson(picJson, new TypeToken<ArrayList<HotelPicJsonBean>>() {}.getType());
    ...
    public Map<String, Object> readonlyRoomTypeInfo(Long roomtypeid ){
	    ...
	    //List<TFacilityModel> tFacs = tFacilityMapper.findByRoomtypeId(roomtypeid, TFacilityMapper.ROOM_TYPE_BINDING);
	...
	public Map<String, Object> readonlyHotelPics(Long hotelId) {
		...
		for(Map<String, String> hm : hotels){
			if( hm == null ) continue;
	         ...
	         if(StringUtils.isBlank(tname)) tname="";
	         if (!StringUtils.isBlank(hotelpic)) {
	             try {
