...
//					iMessageService.sendMsg(phone, StringUtils.getMessage(TPL_SMS_DEF,""), MessageTypeEnum.normal);
		...
		if(PromotionGenTypeEnum.ALL.equals(bActivity.getGentype())){
			if(proList!=null && proList.size()<=4){
				...
				for(BPromotion tmp: proList){
					rtnList.addAll(genCGTicket(tmp.getId(), mid, bActivity.getBegintime(), bActivity.getEndtime(), bActivity.getPromotionmethodtype(),null,null));
				}
			} else {
			...
			if(selection!=null && selection.size()>0){
				...
				rtnList.addAll(genCGTicket(bp.getId(), mid, bActivity.getBegintime(), bActivity.getEndtime(), bActivity.getPromotionmethodtype(),null,null));
			}
		} else {
	 ...
	 */
	@Override
	public List<BPrizeInfo> genTicketByActive(Long activeid, Long mid,String ostype){
		logger.info(">>>根据活动发放优惠券----------------------//开始");
	...
	@Override
	public List<Long> genCGTicket(long promotionid, long mid, Date starttime, Date endtime, PromotionMethodTypeEnum promotionMethodType,String sourcecdkey,Long channelid){
		...
		try{
			...
			if(bp.getOnlineprice()!=null || bp.getOfflineprice()!=null){
				...
				newp.setType(PromotionTypeEnum.immReduce);
				...
				if(genpromotionid!=null){
					...
					//4. 记录用户领券记录
					logMemberGetPromotion(mid, bp.getActivitiesid(), bp.getId(), genpromotionid, "", "", true);
				...
				// 原先逻辑
				genList = oldGenTicket(bp.getId(), bp, starttime, endtime, mid, promotionMethodType,sourcecdkey,channelid);
				logger.info(">>>生成优惠券［旧］业务逻辑---------------------->>开始");
			}
			return genList;
		}catch(Exception e){
			logger.error("genCGTicket(long promotionid, long mid) occur error.", e);
			logMemberGetPromotion(mid, bp.getActivitiesid(), promotionid, null, "", e.getMessage(), false);
		}
		return genList;
	}
	
	@Deprecated
	private List<Long> oldGenTicket(Long promotionid, BPromotion bp, Date starttime, Date endtime, Long mid, PromotionMethodTypeEnum promotionMethodType,String sourcecdkey,Long channelid){
		List<Long> genList = Lists.newArrayList();
		...
		for(CommonXmlPromo cxp: cxpList){
			...
			if(bp.getNum()==-1 || bp.getNum()>0){
				...
				bp.setType(PromotionTypeEnum.immReduce);
				...
				if(genpromotionid!=null){
					...
					logMemberGetPromotion(mid, bp.getActivitiesid(), promotionid, genpromotionid, "", "", true);
					genList.add(genpromotionid);
				}
			} 
		}
	 ...
	 * @param success
	 */
	private void logMemberGetPromotion(Long mid, Long activeid, Long promoinstanceid, Long promotionid, String noticetype, String errormsg, boolean success){
