...
import com.mk.ots.mapper.RoomSaleTypeMapper;
...
public class NewPMSHotelServiceImpl implements NewPMSHotelService {
	...
	public Map<String, Object> syncHotelInfo(JSONObject jsonOBJ) {
		...
		try {
			...
			if (logger.isInfoEnabled()) {
				logger.info("about to update hotelroonnum hotelid {}, roomNum {}", hotelId, roomNum);
			}

			this.updateHotelRoomNum(tHotel, eHotel, roomNum, hotelPhone);

			boolean isSucced = this.syncRoomTypes(hotelId, eHotel.getHotelname(), roomNum,
					pmsHotelInfoJsonBean.getRoomtype());
			if (isSucced) {
				// 需要废弃
				if (logger.isInfoEnabled()) {
					logger.info("about to update mkprice and cache");
				}				
	...
	/**
	 * 判断是否需要做同步操作 1.房间数为0不做同步操作 2.房型门市价为0 不做同步操作
	 */
	 ...
	 */
	private boolean syncRoomTypes(Long hotelId, String hotelName, int roomCount,
			List<PMSRoomTypeBean> pmsRoomTypeList) {
		...
		try {
			...
			logger.error("failed to del/traverse roomtypes", ex);
			return false;
		}
		 ...
		 * end
		 */
		// tRoomtypeList 转为 map<pms,TRoomType>,直接使用参数pms号取map key
		// 如果取到就是更新，如果没有就新增
			...
			try {
				...
				if (null == tRoomTypes) {
					...
					if (pmsRoomTypeBean.getRoom() == null || pmsRoomTypeBean.getRoom().size() == 0) {
						logger.info("PMS2.0 同步房型数据-->开始添加房型：参数roomtype:{} 房间数为0 ,此房型不做同步",
								JsonKit.toJson(pmsRoomTypeBean.getId()));
					} else {
					...
					for (TRoomTypeModel tRoomType : tRoomTypes) {
						logger.info("PMS2.0 同步房型数据-->开始更新房型：参数roomtype:{},tRoomType:{}",
								JsonKit.toJson(pmsRoomTypeBean), JsonKit.toJson(tRoomType));
						roomCount = roomCount + pmsRoomTypeBean.getRoom().size();
						if (pmsRoomTypeBean.getRoom() == null || pmsRoomTypeBean.getRoom().size() == 0) {
							logger.info("PMS2.0 同步房型数据-->开始添加房型：参数roomtype:{} 房间数为0 ,此房型不做同步",
									JsonKit.toJson(pmsRoomTypeBean.getId()));
				...
				logger.error(String.format("failed to update room for pms %s, hotelid %s", hotelId, pms), ex);
			}
		...
		logger.info("delRoomNoLog{},addRoomNoLog{},delRoomTypeLog", delRoomNoLog.size(), addRoomNoLog.size(),
				delRoomTypeLog.size());
	...
	private void updateHotelRoomNum(THotelModel tHotel, EHotelModel eHotel, int roomNum, String hotelPhone) {
		...
		if (eHotel != null) {
			...
			if (eHotel.getPmsstatus() == null || eHotel.getPmsstatus() == PmsStatusEnum.init.getId()
					|| eHotel.getPmsstatus() == PmsStatusEnum.find.getId()) {
	...
	private void updateRoomType(TRoomTypeModel tRoomType, PMSRoomTypeBean pmsRoomTypeBean, List<TRoomModel> tRoomList,
			Map changemap, boolean isOnSaleRoomType) {
		// 非特价房更新房型名称
		if (!isOnSaleRoomType) {
	 ...
	 * @param tRoomType
	 */
	private void updateRooms(TRoomTypeModel tRoomType, List<PMSRoomBean> pmsRoomList, List<TRoomModel> tRoomList,
			Map changemap, boolean isOnSaleRoomType) {
		// 特价房型不更新房型
		if (isOnSaleRoomType) {
			return;
	...
	/**
	 * 添加房型附加信息 e_roomtype_facility e_roomtype_info t_roomtype_info
	 * 
	 * @param tRoomType
	 */
	private void addRoomTypeInfo(TRoomTypeModel tRoomType) {
		...
		if (StringUtils.isNotBlank(initids)) {
			...
			// List<TFacilityModel> tFacilityModelList =
			// tFacilityMapper.findByIds(ids);
