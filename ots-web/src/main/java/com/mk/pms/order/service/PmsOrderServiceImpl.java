...
public class PmsOrderServiceImpl implements PmsOrderService {
    ...
    private boolean checkCacheOrder(Map customNo, Long hotelId) {
        ...
        try {
            ...
            if (jedis.exists(keyStr)) {
                PmsOrderServiceImpl.logger.info("同步pms客单，校验缓存存在相同客单，hotelId=" + hotelId + ",orderString=" + orderString + ",redisKeyStr=" + keyStr);
                return false;
            }
        } catch (Exception e) {
        } finally {
    ...
    private void setCacheOrder(Map customNo, Long hotelId) {
        ...
        try {
            ...
            keyStr = "saveCustomerNo_" + hotelId + "_" + customNo.get("customerno") + "_" + keyStr;
            // save cache
            PmsOrderServiceImpl.logger.info("setCacheOrder:::hotelId=" + hotelId + ",orderString=" + orderString + ",redisKeyStr=" + keyStr);
    ...
    private String genOrderString(Map customNo, Long hotelId) {
        String[] orderKey = { "customerno", "optype", "price", "cost", "recstatus", "arrivetime", "excheckouttime", "priceType", "roominguser", "payment", "orderid", "roomno", "name", "cardid",
                "ethnic", "cardtype", "fromaddress", "address" };
    ...
    private void saveHotelRoomStatusTest(String uuid, Long hotelId, Map customNo, PmsRoomOrder order, int flag) {
        try {
            ...
            if (hotelId == id) {
                ...
                if (flag == 0) {
                    Db.update("insert into hotel_roomstatus_test (uuid,hotelid,customerno,status1,createtime) values('" + uuid + "'," + hotelId + ",'" + customerno + "','" + recstatus + "','" + time
                            + "')");
                } else if (flag == 1) {
                    long count = Db.queryLong("select count(1) from hotel_roomstatus_test where hotelid = " + hotelId + " and uuid = '" + uuid + "' and customerno = '" + customerno + "'");
                    if (count > 0) {
                        Db.update("update  hotel_roomstatus_test set status2 = '" + pmsstatus + "' where uuid = '" + uuid + "' and customerno = '" + customerno + "'");
                    } else {
                        Db.update("insert into hotel_roomstatus_test (uuid,hotelid,customerno,status2,createtime) values('" + uuid + "'," + hotelId + ",'" + customerno + "','" + pmsstatus + "','"
                                + time + "')");
                    }
                    ...
                    ReturnObject<List<CustomerResult>> ro = HotelPMSManager.getInstance().getService().selectCustomerno(hotelId, pmsOrderIds);
                    if ((ro != null) && (ro.getIsError() == false)) {
                        PmsOrderServiceImpl.logger.info("synPmsOrder::call pms ::selectCustomerno::ok::saveResult,返回size{}", ro.getValue().size());
                        try {
                            for (CustomerResult customerResult : ro.getValue()) {
                                long count = Db.queryLong("select count(1) from hotel_roomstatus_test where hotelid = " + hotelId + " and uuid = '" + uuid + "' and customerno = '" + customerno + "'");
                                if (count > 0) {
                                    Db.update("update  hotel_roomstatus_test set status3 = '" + customerResult.getRecstatus() + "' where uuid = '" + uuid + "' and customerno = '" + customerno + "'");
                                } else {
                                    Db.update("insert into hotel_roomstatus_test (uuid,hotelid,customerno,status3,createtime) values('" + uuid + "'," + hotelId + ",'" + customerno + "','"
                        ...
                        PmsOrderServiceImpl.logger.info("同步pms异常test::call pms ::selectCustomerno::err::{},ro.getIsError() = {}", ro.getErrorMessage(), ro.getIsError());
                    }
                }
            }
        } catch (Exception e) {
        ...
        try {
            ...
            for (Map customNo : datalist) {
                ...
                if (customNo.containsKey("optype") && customNo.get("optype").equals("delete")) {
                    ...
                    PmsRoomOrder order = this.hotelDAO.findPmsRoomOrderByHotelIdAndCustomno(hotelId, (String) customNo.get("customerno"));
                    if (order == null) {
                        PmsOrderServiceImpl.logger.warn("PMS要求取消一条不存在的客单记录，酒店id：" + hotelId + "中 酒店预客单号为：" + (String) customNo.get("customerno"));
                    } else {
                    ...
                    try {
                        ...
                        PmsOrderServiceImpl.logger.info("PmsOrderServiceImpl::saveCustomerNo::updatePmsRoomOrder重复数据插入:order:{}",order);
                        if (order == null) {
                            continue;
                        }
                    ...
                    this.setCacheOrder(customNo, hotelId);
                    // 记录身份证、卡号、民族 数据
                    if (!"RE".equals(customNo.get("recstatus")) && StringUtils.isNotBlank((String) customNo.get("cardid")) && StringUtils.isNotBlank((String) customNo.get("cardtype"))
                        ...
                        PmsOrderServiceImpl.logger.info("OTSMessage::PmsOrderServiceImpl::checkinService::saveOrUpdatePmsInCheckUser::参数::" + checkinUser.getAttrs());
    ...
    private PmsRoomOrder updatePmsRoomOrder(EHotel hotel, PmsRoomOrder newOrder, Map customNo, Long hotelId) throws Exception {
        String pmsStates = "RE,RX,IN,IX,PM,OK";
        ...
        if (StringUtils.isNotBlank(order.getStr("Status")) && StringUtils.isNotBlank((String) customNo.get("recstatus"))
                && (pmsStates.indexOf((String) customNo.get("recstatus")) < pmsStates.indexOf(order.getStr("Status")))) {
            PmsOrderServiceImpl.logger.info("PmsOrderServiceImpl::saveCustomerNo::错误PMS状态，当前客单状态已经是{},不能再{}", order.getStr("Status"), customNo.get("recstatus"));
            return null;
        }
        ...
        if (PmsRoomOrderStatusEnum.RE.getId().equals(customNo.get("recstatus"))) {
            ...
            // 续住的场景
            if ((customNo.get("excheckouttime") != null) && !customNo.get("excheckouttime").equals(order.get("EndTime"))) {
                order.set("EndTime", customNo.get("excheckouttime"));
            }
        ...
        } else if (PmsRoomOrderStatusEnum.OK.getId().equals(customNo.get("recstatus")) || PmsRoomOrderStatusEnum.PM.getId().equals(customNo.get("recstatus"))) {
            if (customNo.get("excheckouttime") != null) {
    ...
    public void ChangeOtaOrder(List<PmsRoomOrder> pmsRoomOrderList, Map unlockParam) {
        ...
        for (PmsRoomOrder pmsRoomOrder : pmsRoomOrderList) {
            ...
            PmsOrderServiceImpl.logger.info("OTSMessage::changePmsRoomOrder-修改ota客单:orderid:{}-{}", otaorderid, roomOrder);
            if (roomOrder != null) {
                ...
                if (!roomOrder.get("RoomId").equals(changeRoomOrderBean.getRoomId())) {
                    ...
                    PmsOrderServiceImpl.logger.info("OTSMessage::changePmsRoomOrder-换房otaorderid::{},ota客单-{}", roomOrder.get("otaorderid"), roomOrder);
                    ...
                    PmsOrderServiceImpl.logger.info("changePmsRoomOrder::roomid:{},oldroomid:{}", changeRoomOrderBean.getRoomId(), changeRoomOrderBean.getOldRoomId());
                    if (changeRoomOrderBean.getOldRoomId() != null && !changeRoomOrderBean.getOldRoomId().equals(changeRoomOrderBean.getRoomId())) {
                        PmsOrderServiceImpl.logger.info("changePmsRoomOrder::otaorderid:{},换房信息:changeRoom:{}", roomOrder.get("otaorderid"), changeRoom);
        ...
        if (unlockParam.size() == 3) {
            ...
            // } catch (Exception e) {
            // logger.error("PmsCalCacheEvent清楚缓存：hotelid:"+entry.getKey()+" roomTypeID:"+id,
            // e);
            // }
            // }
            // }
    ...
    public Map synOrder(Map param) {
        ...
        try {
            ...
            PmsOrderServiceImpl.logger.info("synOrder::synLockKey:" + synLockKey + "-----currentDate:" + sdf.format(new Date()));
            // 加redis锁，防止重复请求
            ...
            PmsOrderServiceImpl.logger.info("OTSMessage::resetRoomStatusByHotelidFromPMS::synOrder::全量更新::hotelid:{}", hotelId);
    ...
    private List<TRoomRepair> saveRepairs(Long hotelId, List<Map> datalist) {
        ...
        for (Map repair : datalist) {
            if (repair.containsKey("optype") && repair.get("optype").equals("delete")) {
                ...
                if (old == null) {
                    PmsOrderServiceImpl.logger.warn("PMS要求取消一条不存在的维修单记录，酒店id：" + hotelId + "中 维修单号为：" + (String) repair.get("id"));
                } else {
                    old.delete();
                    returnlist.add(old);
                }
    ...
    public Map orderCharge(Map param) {
        ...
        try {
            ...
            for (Map orderCharge : customerno) {
                PmsRoomOrder order = this.hotelDAO.findPmsRoomOrderByHotelIdAndCustomno(hotelId, (String) orderCharge.get("custmoerid"));
                if (order == null) {
                    PmsOrderServiceImpl.logger.warn("PMS要求结算一条不存在的客单记录，酒店id：" + hotelId + "中 酒店客单号为：" + orderCharge.get("custmoerid") + "");
                } else {
    ...
    public List<PmsOrder> savePmsRoomTypeOrder(Long hotelId, List<Map> dataList) {
        ...
        PmsOrderServiceImpl.logger.info("OTSMessage::PmsOrderServiceImpl::savePmsRoomTypeOrder::更新[" + hotel.getHotelName() + "]酒店的预订单记录");
        ...
        for (Map roomTypeOrder : dataList) {
            ...
            if (roomTypeOrder.containsKey("optype") && roomTypeOrder.get("optype").equals("delete")) {
                PmsOrder old = this.hotelDAO.findPmsOrderByHotelIdAndPmsOrderNo(hotelId, (String) roomTypeOrder.get("orderid"), (String) roomTypeOrder.get("batchno"));
                if (old == null) {
                    PmsOrderServiceImpl.logger.warn("PMS要求取消一条不存在的预订单记录，酒店id：" + hotelId + "中 酒店预订单号为：" + roomTypeOrder.get("orderid") + "的批次号" + roomTypeOrder.get("batchno") + "");
                } else {
                    old.set("Cancel", "T");
                    old.set("Visible", "T");
                    old.update();
    ...
    public Map savePmsRoomTypeOrder(Map param) {
        ...
        try {
            ...
            PmsOrderServiceImpl.logger.info("OTSMessage::PmsOrderServiceImpl::call::savePmsRoomTypeOrder::更新[" + hotel.getHotelName() + "]酒店的预订单记录::hotelId::" + hotelId);
            List<Map> dataList = (List<Map>) param.get("datalist");
            PmsOrderServiceImpl.logger.info("OTSMessage::PmsOrderServiceImpl::call::savePmsRoomTypeOrder::dataList::" + dataList);
            List<PmsOrder> returnlist = this.savePmsRoomTypeOrder(hotelId, dataList);
            PmsOrderServiceImpl.logger.info("OTSMessage::PmsOrderServiceImpl::call::flushCacheByHotelidAndRoomTypeId::" + returnlist);
    ...
    private void flushCacheByHotelidAndRoomTypeId(List<PmsOrder> returnlist) {
        ...
        for (PmsOrder pmsOrder : returnlist) {
            PmsOrderServiceImpl.logger.info("savePmsRoomTypeOrder清楚缓存：hotelid:" + pmsOrder.getLong("HotelId") + "roomtypeid:" + pmsOrder.getLong("RoomTypeId"));
            try {
                EventBusHelper.getEventBus().post(new PmsCalCacheEvent(pmsOrder.getLong("HotelId"), pmsOrder.getLong("RoomTypeId")));
            } catch (Exception e) {
                PmsOrderServiceImpl.logger.error("savePmsRoomTypeOrder清楚缓存：hotelid:" + pmsOrder.getLong("HotelId") + "roomtypeid:" + pmsOrder.getLong("RoomTypeId"));
            }
        }
    }
    ...
    public Map cancelOrder(Map param) {
        ...
        try {
            EHotel hotel = this.eHotelDAO.findEHotelByid(hotelId);
            PmsOrderServiceImpl.logger.debug("OTSMessage::PmsOrderServiceImpl::call::cancelOrder::取消[" + hotel.getHotelName() + "]酒店的预订单记录");
            List<PmsOrder> returnlist = new ArrayList<PmsOrder>();
            List<Map> datalist = (List<Map>) param.get("datalist");
            for (Map cancelBean : datalist) {
                List<PmsOrder> list = this.hotelDAO.findPmsOrderByHotelIdAndPmsOrderNo(hotelId, Long.parseLong((String) cancelBean.get("orderidpms")));
                returnlist.addAll(list);
                for (PmsOrder pmsOrder : list) {
                    pmsOrder.set("cancel", "T");
                    pmsOrder.saveOrUpdate();
                    PmsOrderServiceImpl.logger.info("OTSMessage::PmsOrderServiceImpl::cancelOrder::pmsOrder.saveOrUpdate()");
                }
                this.flushCacheByHotelidAndRoomTypeId(returnlist);
            }
            data.put("success", true);
        ...
        try {
            ...
            for (Map repair : datalist) {
                ...
                if (repair.containsKey("delete") && delete) {
                    TRoomRepair old = this.hotelDAO.findTRoomRepairByHotelIdAndRepairId(hotelId, (String) repair.get("id"));
                    if (old == null) {
                        PmsOrderServiceImpl.logger.info("PMS要求取消一条不存在的维修单记录，酒店id：" + hotelId + "中 维修单号为：" + repair.get("id"));
                    } else {
                        TRoomRepair roomRepair = TRoomRepair.dao.findById(Long.parseLong(String.valueOf(old.get("id"))));
                        // 房子修好了，则删除ots里的记录
                        roomRepair.delete();
                        returnlist.add(old);
                    }
            ...
            for (TRoomRepair tRoomRepair : returnlist) {
                EventBusHelper.getEventBus().post(new PmsCalCacheEvent(tRoomRepair.getLong("hotelid"), tRoomRepair.getLong("roomtypeid")));
            }
            data.put("success", true);
        } catch (Exception e) {
            data.put("success", false);
    ...
    public void synPmsOrder(Long hotelid, String roomNos) {
        PmsOrderServiceImpl.logger.info("OTSMessage::resetRoomStatusByHotelidFromPMS::synPmsOrder::全量更新::同步消息:{}", hotelid);
        ...
        if (oldRoomOrderlist.size() > 0) {
            ...
            if ("T".equals(hotel.getStr("isNewPms"))) {
                ...
                PmsOrderServiceImpl.logger.info("synPmsOrder::call pms ::selectCustomerno::ok::saveResult,同步今天0点之后数据传参{}{}", hotelid, pmsCustomNos.toString());
                if ((ro != null) && (ro.getIsError() == false)) {
                    PmsOrderServiceImpl.logger.info("synPmsOrder::call pms ::selectCustomerno::ok::saveResult,返回size{}", ro.getValue().size());
                    ...
                    try {
                        ...
                        PmsOrderServiceImpl.logger.info("synPmsOrder::call pms ::selectCustomerno::ok::saveResult,同步今天0点之后数据{}{}", hotelid, theResult);
                    } catch (Exception e) {
                        PmsOrderServiceImpl.logger.error("同步今天0点之后数据错误:{}", e.getMessage());
                    }
                } else {
                    PmsOrderServiceImpl.logger.error("synPmsOrder::call pms ::selectCustomerno::err::{}", ro.getErrorMessage());
                }
            }
        }
    }
    ...
    public String synPmsOrderBefore(Long hotelid, String roomNos) {
		 ...
		 * logger.info
		 * ("synPmsOrder::call pms selectOrder::pmsOrderIds::"+pmsOrderIds);
		 * ReturnObject<List<OrderResult>> returnobject =
		 * HotelPMSManager.getInstance().getService().selectOrder(hotelid,
		 * pmsOrderIds);
		 * logger.info("synPmsOrder::call pms selectOrder::pmsOrderIds::ok"); //
        ...
        if (oldRoomOrderlist.size() > 0) {
            ...
            for (String k : pmsCustomNos) {
                ...
                list.add(k);
                ReturnObject<List<CustomerResult>> ro = HotelPMSManager.getInstance().getService().selectCustomerno(hotelid, list);
				/*
				 ...
				 */
                PmsOrderServiceImpl.logger.info("synPmsOrder::call pms ::selectCustomerno::ok::saveResult,同步今天之前数据传参{}{}", hotelid, pmsCustomNos.toString());
                if ((ro != null) && (ro.getIsError() == false)) {
                    PmsOrderServiceImpl.logger.info("synPmsOrder::call pms ::selectCustomerno::ok::saveResult,返回size{}", ro.getValue().size());
                    String theResult = this.saveResult(hotelid, ro.getValue());
                    result += ("~~~" + theResult);
                    PmsOrderServiceImpl.logger.info("synPmsOrder::call pms ::selectCustomerno::ok::saveResult,同步今天之前数据{}{}", hotelid, theResult);
                } else {
                    PmsOrderServiceImpl.logger.info("synPmsOrder::call pms ::selectCustomerno::err::{}", ro.getErrorMessage());
                }
            }
        }
        return result;
    ...
    public String saveResult(Long hotelId, List<CustomerResult> result) {
		...
		/*if (map.containsKey("set")) {
			Map<Long, Set<Long>> map_ = (Map<Long, Set<Long>>) map.get("set");
			// 计算缓存
			for (Object _hotelId : map_.keySet()) {
				Set<Long> roomTypeIds = map_.get(_hotelId);
				for (Long roomTypeId : roomTypeIds) {
					this.roomService.calCacheByHotelIdAndRoomType((Long) _hotelId, roomTypeId);
				}
			}
		}*/
    ...
    private void saveResult(Long hotelId, OrderResult or, List<PmsOrder> oldlist) {
        ...
        if (or.getRoomTypeOrderList() != null) {
			...
			/*for (PmsOrder pmsOrder : sendlist) {
				HotelService hotelService = AppUtils.getBean(HotelService.class);
				this.roomService.calCacheByHotelIdAndRoomType(pmsOrder.getLong("Hotelid"), pmsOrder.getLong("roomtypepms"));
			}*/
    ...
    private void removeOldList(List<PmsOrder> oldlist, Map rto) {
        ...
        while (iter.hasNext()) {
            ...
            if (StringUtils.equals(String.valueOf(old.get("PmsOrderNo")), String.valueOf(rto.get("orderid")))
                    && StringUtils.equals(String.valueOf(old.get("PmsRoomTypeOrderNo")), String.valueOf(rto.get("batchno")))) {
                iter.remove();
            }
        }
    }
